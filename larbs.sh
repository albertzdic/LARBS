...
# To disable touchpad
dialog --title "Message"  --yesno "Do you want to\ disable the touchpad?" 6 25 || echo "exec_always --no-startup-id xinput disable $(echo "*$(xinput list --name-only | grep -i touchpad)*" | sed s/^\*/\'/g | sed s/\*$/\'/g )" >> /home/$name/.config/i3/config ; clear
